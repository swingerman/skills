...
Bundled statusline showing **model · context-window usage · rate limits · worktree+branch · effort level**, with **eight visual themes** to pick from.
...
| `catppuccin-mocha` | **Catppuccin Mocha–branded**: same chrome as `panels` (rounded caps,  arrows, icons), but the alternating Surface0/Surface1 backgrounds and per-segment accent foregrounds (Sapphire context, Yellow 5h, Green 7d, Sky worktree, Mauve branch, Peach effort, Text model) come from the official Catppuccin palette via 24-bit color codes. Coordinates visually with the OMP `catppuccin_mocha.omp.json` prompt. | ❌ (uses fixed Catppuccin RGB values regardless of terminal palette — pair with a Catppuccin terminal scheme for the best look) |
| `catppuccin-macchiato` | Same as `catppuccin-mocha` but with the slightly lighter Macchiato palette. Pairs with the bundled `catppuccin.omp.json` prompt (which uses Macchiato Blue/Pink/Lavender pastels) and the WezTerm/Ghostty `Catppuccin Macchiato` color schemes. | ❌ (fixed Macchiato RGB values) |
...
Run the bundled preview script — renders all eight themes with sample data:
...
```sh
THEME=pure   # or: powerline | rainbow | terminal | panels | catppuccin-mocha | catppuccin-macchiato | minimal
tmp=$(mktemp)
